    ...
    return (
        <section className="w-full flex flex-row h-full pb-12 overflow-hidden">
            ...
            <div className="w-full 2xl:w-5/12 flex h-full justify-center items-end flex-col">
                ...
                <ContactButton text={t.contactModal.emailMe} href={`mailto:${t.contactModal.email}`} />
                <ContactButton text={t.contactModal.linkedin} href={Links.linkedin} />
                <ContactButton text={`${t.contactModal.discord} ${Links.discord}`} className="!text-xl" />
            </div>
        </section>
    )
}

const ContactButton = ({ text, href, className }: { text: string, href?: string, className?: string }) => {
    return (
        <a href={href} className={`${className} text-center group flex relative items-center justify-center hover:bg-blue-500 text-white my-4 w-full cursor-pointer uppercase rounded-full py-6 bg-blue-600 font-bold text-lg sm:text-3xl`}>
